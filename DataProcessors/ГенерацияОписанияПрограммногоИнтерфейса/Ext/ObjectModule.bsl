...
Процедура ПрочитатьМетод(Строка, ТекстовыйДокумент, Итератор, ОшибкаЧтенияМетода, Рекурсия = Ложь)
	...
	ИначеЕсли СтрНачинаетсяС(Строка, СтрокаПроцедура() + " ") > 0 Или СтрНачинаетсяС(Строка, СтрокаФункция() + " ") > 0
		Или СтрНачинаетсяС(Строка, СтрокаПроцедураАсинх() + " ") > 0 Или СтрНачинаетсяС(Строка, СтрокаФункцияАсинх() + " ") > 0 Тогда
...
Функция СинтаксисВызоваМетода(Знач Метод, Знач Расположение)
	...
	Метод = СтрЗаменить(Метод, СтрокаФункцияАсинх() + " ", "");
	Метод = СтрЗаменить(Метод, СтрокаПроцедураАсинх() + " ", "");
...
Функция СтрокаФункцияАсинх()
	
	Если АнглийскаяВерсия Тогда
		Возврат "Async Function";//@Non-NLS
	Иначе
		Возврат "Асинх Функция";//@Non-NLS
	КонецЕсли;
...
Функция СтрокаПроцедураАсинх()
	
	Если АнглийскаяВерсия Тогда
		Возврат "Async Procedure";//@Non-NLS
	Иначе
		Возврат "Асинх Процедура";//@Non-NLS
	КонецЕсли;
