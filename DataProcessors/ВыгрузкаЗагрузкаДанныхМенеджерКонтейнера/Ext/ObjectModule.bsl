...
Процедура ФайлЗаписан(Знач ПолныйПутьКФайлу) Экспорт
	...
	Если СтрокаСостава <> Неопределено Тогда
		СтрокаСостава.Размер = РазмерФайла;
	Иначе
		РазмерПроизвольногоФайла = РазмерФайла;
	КонецЕсли;
	
	ОтносительноеИмя = Сред(ПолныйПутьКФайлу, СтрДлина(ВременныйКаталог));
	КаталогАрхива = ПолучитьИмяВременногоФайла("zip"); //@skip-check missing-temporary-file-deletion
	ПолныйПутьКФайлуВКаталогеАрхива = КаталогАрхива + ОтносительноеИмя;
	...
	ПереместитьФайл(ПолныйПутьКФайлу, ПолныйПутьКФайлуВКаталогеАрхива);
	...
	Если ЭтоДочернийПоток() Тогда
		...
		ПараметрыЗаписанныхФайлов = Параметры.ПараметрыЗаписанныхФайлов; // см. НовыеПараметрыЗаписанныхФайловВПотоке
		ПараметрыЗаписанныхФайлов.РазмерФайлов = ПараметрыЗаписанныхФайлов.РазмерФайлов + РазмерФайла;
		
		СтрокаТаблицыФайлов = ПараметрыЗаписанныхФайлов.Файлы.Добавить();
		СтрокаТаблицыФайлов.ПолныйПутьКФайлу = ПолныйПутьКФайлуВКаталогеАрхива;
		СтрокаТаблицыФайлов.Размер = РазмерФайла;
		
		// Ждем пока родительский поток обработает записанные файлы, чтобы не потреблять лишнее место во временных файлах
		НомерПроверки = 0;
		МаксимальнаяПауза = 5; // секунд
		
		Пока ЗаписанныеФайлыПревышаютДопустимыйРазмер(ПараметрыЗаписанныхФайлов) Цикл
			
			ДлительностьПаузы = Мин(1 + 0.1 * НомерПроверки, МаксимальнаяПауза);
...
Процедура ИнициализироватьПеременныеВПотоке(ПараметрыКонтейнера)
	...
	Параметры = ПараметрыКонтейнера.Параметры; // Структура
	
	Если ДляЗагрузки Тогда
		Состав = ПараметрыКонтейнера.Состав;
	ИначеЕсли ДляВыгрузки Тогда
		Параметры.Вставить("ПараметрыЗаписанныхФайлов", НовыеПараметрыЗаписанныхФайловВПотоке());
	КонецЕсли;
...
// Параметры:
//  ПараметрыЗаписанныхФайлов - см. НовыеПараметрыЗаписанныхФайловВПотоке
// 
// Возвращаемое значение:
//  Булево
Функция ЗаписанныеФайлыПревышаютДопустимыйРазмер(ПараметрыЗаписанныхФайлов)
	
	ДопустимыйРазмер = 1073741824; // 1 Гб.
	
	Если ПараметрыЗаписанныхФайлов.РазмерФайлов < ДопустимыйРазмер Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПараметрыЗаписанныхФайлов.РазмерФайлов = 0;
	ТаблицаФайлов = ПараметрыЗаписанныхФайлов.Файлы;
	
	// Проверим фактическое наличие файлов, если их нет, значит родительский поток уже обработал их и удалил
	Для ОбратныйИндекс = 1 - ТаблицаФайлов.Количество() По 0 Цикл
		
		СтрокаТаблицыФайлов = ТаблицаФайлов[-ОбратныйИндекс];
		Файл = Новый Файл(СтрокаТаблицыФайлов.ПолныйПутьКФайлу);
		
		Если Файл.Существует() Тогда
			ПараметрыЗаписанныхФайлов.РазмерФайлов = ПараметрыЗаписанныхФайлов.РазмерФайлов + СтрокаТаблицыФайлов.Размер;
		Иначе
			ТаблицаФайлов.Удалить(-ОбратныйИндекс);
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ПараметрыЗаписанныхФайлов.РазмерФайлов > ДопустимыйРазмер;
...
// ** Размер - Число
Функция НовыеПараметрыЗаписанныхФайловВПотоке()
	
	ТаблицаФайлов = Новый ТаблицаЗначений();
	ТаблицаФайлов.Колонки.Добавить(
		"ПолныйПутьКФайлу",
	...
	ПараметрыЗаписанныхФайлов = Новый Структура();
	ПараметрыЗаписанныхФайлов.Вставить("РазмерФайлов", 0);
	ПараметрыЗаписанныхФайлов.Вставить("Файлы", ТаблицаФайлов);
	
	Возврат ПараметрыЗаписанныхФайлов;
