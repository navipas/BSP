...
Процедура ВыполнитьОбработчикиСобытийПерехода(Знач ЭтоПереходДалее)
	...
	Если ЭтоПереходДалее Тогда
		...
		Если СтрокиПерехода.Количество() > 0 Тогда
			...
			Если Не ПустаяСтрока(СтрокаПерехода.ИмяОбработчикаПриПереходеДалее)
				...
				ИмяПроцедуры = "Подключаемый_" + СтрокаПерехода.ИмяОбработчикаПриПереходеДалее;
				Отказ = Ложь;
				
				ПроверитьИмяПроцедуры(ИмяПроцедуры);
				Результат = Вычислить(ИмяПроцедуры + "(Отказ)"); // Не нарушает безопасность информации, так как вычисляется только функция модуля.
	...
	Иначе
		...
		Если СтрокиПерехода.Количество() > 0 Тогда
			...
			Если Не ПустаяСтрока(СтрокаПерехода.ИмяОбработчикаПриПереходеНазад)
				...
				ИмяПроцедуры = "Подключаемый_" + СтрокаПерехода.ИмяОбработчикаПриПереходеНазад;
				Отказ = Ложь;
	...
	Если Не ПустаяСтрока(СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии) Тогда
		...
		ИмяПроцедуры = "Подключаемый_" + СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии;
		Отказ = Ложь;
		ПропуститьСтраницу = Ложь;
		
		ПроверитьИмяПроцедуры(ИмяПроцедуры);
		Результат = Вычислить(ИмяПроцедуры + "(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)"); // Не нарушает безопасность информации, так как вычисляется только функция модуля.
...
Процедура ВыполнитьОбработчикДлительнойОперации()
	...
	Если Не ПустаяСтрока(СтрокаПереходаТекущая.ИмяОбработчикаДлительнойОперации) Тогда
		...
		ИмяПроцедуры = "Подключаемый_" + СтрокаПереходаТекущая.ИмяОбработчикаДлительнойОперации;
		Отказ = Ложь;
		ПерейтиДалее = Истина;
		
		ПроверитьИмяПроцедуры(ИмяПроцедуры);
		Результат = Вычислить(ИмяПроцедуры + "(Отказ, ПерейтиДалее)"); // Не нарушает безопасность информации, так как вычисляется только функция модуля.
...
&НаКлиенте
Процедура ПроверитьИмяПроцедуры(ИмяПроцедуры)
