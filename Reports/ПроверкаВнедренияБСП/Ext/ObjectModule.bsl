...
Процедура Подключаемый_КонтактнаяИнформация_ПроверитьВнедрение()
	...
	Для каждого СтрокаТаблицы Из ДанныеНачальногоЗаполнения.ПредопределенныеДанные Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ИмяПредопределенныхДанных) Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.ИмяПредопределенногоВида) 
			   И СтрСравнить(СтрокаТаблицы.ИмяПредопределенныхДанных, СтрокаТаблицы.ИмяПредопределенногоВида) <> 0 Тогда
				ПодробныйТекстОшибкиВида = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Предопределенный вид контактной информации %1 содержит противоречивые данные.
							|Если заполнены оба реквизита ИмяПредопределенныхДанных и ИмяПредопределенногоВида, - их значения должны совпадать.
							|Установите одинаковые значения в обоих полях или очистите одно из полей.'"), 
					СтрокаТаблицы.ИмяПредопределенныхДанных, ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенныхДанных));
				КраткийТекстОшибкиВида = НСтр("ru = 'Значения реквизитов ИмяПредопределенныхДанных и ИмяПредопределенногоВида должны совпадать, если оба поля заполнены.'");
				
				ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, КраткийТекстОшибкиВида, ПодробныйТекстОшибкиВида);
			КонецЕсли;
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьВнедрение()
	...
	Параметры.УправлениеДоступом.ОтключенСтандартныйВариант = МодульУправлениеДоступомСлужебный.ОтключенСтандартныйВариант();
	Параметры.УправлениеДоступом.ШаблоныОграничений = ЭталонныеШаблоныОграничений(Параметры);
	
	Если ТребуетсяВыполнениеПроверки() Тогда
		ОграниченияДоступа = ОграниченияДоступаИзВыгрузкиКонфигурацииВФайлы();
		ПроверитьНаличиеОграниченияПоВсемПолям(ОграниченияДоступа);
		Если Не Параметры.УправлениеДоступом.ОтключенСтандартныйВариант Тогда
			ПроверитьИспользованиеОграниченийДоступа(ОграниченияДоступа);
		КонецЕсли;
		Если Не Параметры.ОбновитьТолькоШаблоныОграничений Тогда
			ПроверитьСоответствиеСоставаСписковСОграничениемНаличиюRLSВРолях(ОграниченияДоступа,
				Параметры.УправлениеДоступом.СпискиСОграничениемДоступа);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьОшибку(Знач ОбъектМетаданных, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки,
	...
	ЗаполнитьУточнениеПодсистемыКонфигурации(НоваяСтрока, ПодсистемаОбъекта, ОбъектМетаданных);
...
Процедура ЗаполнитьУточнениеПодсистемыКонфигурации(НоваяСтрока, ПодсистемаОбъекта, Знач ОбъектМетаданных)
	
	МетаданныеПодсистемыВерхнегоУровня = ?(ПодсистемаОбъекта = Неопределено, Неопределено,
		Метаданные.Подсистемы.Найти(ПодсистемаОбъекта.Имя));
	
	Если МетаданныеПодсистемыВерхнегоУровня = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Строка(ОбъектМетаданных));
		Если ОбъектМетаданных = Неопределено Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	УточнениеПодсистемы = Неопределено;
	НайденаПовторно = Ложь;
	
	Для Каждого ПодсистемаУровня2 Из МетаданныеПодсистемыВерхнегоУровня.Подсистемы Цикл
		Если ПодсистемаУровня2.Состав.Содержит(ОбъектМетаданных) Тогда
			Если УточнениеПодсистемы <> Неопределено Тогда
				НайденаПовторно = Истина;
				Прервать;
			КонецЕсли;
			УточнениеПодсистемы = ПодсистемаУровня2.Представление();
		КонецЕсли;
		Для Каждого ПодсистемаУровня3 Из ПодсистемаУровня2.Подсистемы Цикл
			Если ПодсистемаУровня3.Состав.Содержит(ОбъектМетаданных) Тогда
				Если УточнениеПодсистемы <> Неопределено Тогда
					НайденаПовторно = Истина;
					Прервать;
				КонецЕсли;
				УточнениеПодсистемы = ПодсистемаУровня2.Представление()
					+ "." + ПодсистемаУровня3.Представление();
			КонецЕсли;
		КонецЦикла;
		Если НайденаПовторно Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если УточнениеПодсистемы <> Неопределено И Не НайденаПовторно Тогда
		НоваяСтрока.УточнениеПодсистемыКонфигурации = УточнениеПодсистемы;
	КонецЕсли;
...
Процедура ПроверитьИспользованиеПроксиВФайле(ПараметрыОшибкиИспользованияПрокси)
	...
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.Прочитать(ПараметрыОшибкиИспользованияПрокси.ПолноеИмяФайла, КодировкаТекста.UTF8);
...
Функция УсловияПоискаМетодовВыполнить()
	...
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ОбщегоНазначения", "ВыполнитьФоновоеЗадание", 1, Истина, Истина));
...
Процедура ПроверитьВызовыМетодовКакПроизвольныйКод()
	...
	ИспользуемыеПроцедурыМодулей = ?(НастройкиПроверки.ОшибкаКогдаУказанныйМетодНеВызывается,
		Новый Соответствие, Неопределено);
	
	Для Каждого КлючИЗначение Из НастройкиПроверки.ИсключенияКогдаУказанныйМетодНеВызывается Цикл
		Для Каждого ОписаниеМетода Из КлючИЗначение.Значение Цикл
			ДобавитьИспользуемуюПроцедуру(ИспользуемыеПроцедурыМодулей,
				КлючИЗначение.Ключ, ОписаниеМетода.Ключ, ОписаниеМетода.Значение = Истина,, Неопределено);
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого Описание Из НастройкиПроверки.ОписаниеОбработчиков Цикл
		ПроверитьВызовыМетодовОбработчиковПодсистем(Описание.ИменаПроцедур,
			Описание.ЗаголовокОшибки, ИспользуемыеПроцедурыМодулей);
	КонецЦикла;
	...
	ДополнительныеПараметры.Вставить("ИспользуемыеПроцедурыМодулей", ИспользуемыеПроцедурыМодулей);
	...
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		...
		ИсключенияДляМодуля = ИсключенияПроверки.Получить(ОбъектМетаданных.ПолноеИмя()
			+ ?(ВРег(ФайлМодуля.ИмяБезРасширения) = ВРег("ObjectModule"), ".МодульОбъекта", ""));
		...
		ДополнительныеПараметры.ПроцедурыМодуля = ?(ИсключенияДляМодуля = Неопределено,
			Неопределено, ПроцедурыМодуля(ТекстМодуля));
		
		ПроверитьВызовыМетодовКакПроизвольныйКодВМодуле(ТекстМодуля, ДополнительныеПараметры);
		ПроверитьИменаУказанныхМетодовМодуля(ТекстМодуля, ФайлМодуля.ИмяБезРасширения, ДополнительныеПараметры);
		
	КонецЦикла;
	
	Если ИспользуемыеПроцедурыМодулей <> Неопределено Тогда
		ПроверитьИспользуемыеПроцедурыМодулей(ИспользуемыеПроцедурыМодулей);
	КонецЕсли;
...
Процедура ПроверитьВызовыМетодовКакПроизвольныйКодВМодуле(ТекстМодуля, ДополнительныеПараметры)
	...
	Для Каждого РезультатПоиска Из РезультатыПоиска Цикл
		...
		ПозицияНачалаПредыдущейСтроки = СтрНайти(ТекстМодуля,
			Символы.ПС, НаправлениеПоиска.СКонца, ПозицияНачалаСтроки - 2) + 1;
		НачалоСтрокиВключаяПредыдущуюСтроку = ?(ПозицияНачалаПредыдущейСтроки = 0, НачалоСтроки, Сред(ТекстМодуля,
			ПозицияНачалаПредыдущейСтроки, ПозицияНачалаСтроки - ПозицияНачалаПредыдущейСтроки + ПозицияВСтроке - 1));
		Если СтрНайти(НачалоСтрокиВключаяПредыдущуюСтроку, ".ФрагментСодержания") > 0 Тогда
			Если ДополнительныеПараметры.ПроцедурыМодуля = Неопределено Тогда
		...
		Если ТекущееУсловиеПоиска = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ВызовЧерезМетодПлатформы = ТекущееУсловиеПоиска.ЭтоФункция = Неопределено;
		
		Если ДополнительныеПараметры.Исключения <> Неопределено Тогда
			...
			Если ИменаПроцедур <> Неопределено Тогда
				Для Каждого ИмяПроцедуры Из ИменаПроцедур Цикл
					...
					Если Размещение = Неопределено
					 Или ВызовЧерезМетодПлатформы
					   И (ИмяПроцедуры.Ключ <> "ВыполнитьФоновоеЗадание"
					      Или ДополнительныеПараметры.ОбъектМетаданных <> Метаданные.ОбщиеМодули.ОбщегоНазначения) Тогда
						Продолжить;
					КонецЕсли;
		...
		ЭтоПараметрЛитерал = СтрНачинаетсяС(Параметр, """") И СтрЗаканчиваетсяНа(Параметр, """");
		
		Если ЭтоПараметрЛитерал
		   И (ТекущееУсловиеПоиска.ИмяМетода = "ВыполнитьМетодОбъекта"
		      Или ВызовЧерезМетодПлатформы) Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтоМетодРегламентногоЗадания = Не ЭтоПараметрЛитерал
			И СтрНачинаетсяС(ВРег(Параметр), ВРег("Метаданные.РегламентныеЗадания"))
			И СтрЗаканчиваетсяНа(ВРег(Параметр), ВРег(".ИмяМетода"));
		
		Если ЭтоМетодРегламентногоЗадания И ВызовЧерезМетодПлатформы Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяМетодаРегламентногоЗадания = Неопределено;
		Если ЭтоМетодРегламентногоЗадания Тогда
			ИмяРегламентногоЗаданияСКонцомСтроки =
				Сред(Параметр, СтрДлина("Метаданные.РегламентныеЗадания") + 2);
			ИмяРегламентногоЗадания = Лев(ИмяРегламентногоЗаданияСКонцомСтроки,
				СтрДлина(ИмяРегламентногоЗаданияСКонцомСтроки) - СтрДлина(".ИмяМетода"));
			МетаданныеЗадания = Метаданные.РегламентныеЗадания.Найти(ИмяРегламентногоЗадания);
			ИмяМетодаРегламентногоЗадания = ?(МетаданныеЗадания = Неопределено,
				Неопределено, МетаданныеЗадания.ИмяМетода);
		КонецЕсли;
		
		ТекстОшибки = Неопределено;
		ТекстИсправленияОшибки = "";
		Если Не ЭтоПараметрЛитерал И ИмяМетодаРегламентногоЗадания = Неопределено Тогда
			Если ТекущееУсловиеПоиска.ИмяМетода = "ВыполнитьФоновоеЗадание" Тогда
				Продолжить;
			КонецЕсли;
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Имя вызываемой процедуры не является строковым литералом и методом регламентного задания в формате ""%1"".'"),
				"Метаданные.РегламентныеЗадания<.ИмяРегламентногоЗадания>.ИмяМетода");
			Если ВызовЧерезМетодПлатформы Тогда
				ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для исправления нужно:
					           |- указать вместо переменной (или выражения) строку полного имени процедуры;
					           |- либо если это вызов метода регламентного задания, тогда указать обращение к свойству метаданных (можно через переменную, если необходимо: %1);
					           |- либо если это универсальный механизм, тогда вернуть имена вызываемых методов в процедуре %2, а само место вызова заменить на вызов через %3.'"),
					"Метаданные.РегламентныеЗадания[ИмяЗадания].ИмяМетода",
					"ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод",
					"ОбщегоНазначения.ВыполнитьФоновоеЗадание");
			Иначе
				ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для исправления нужно:
					           |- указать вместо переменной (или выражения) строку полного имени процедуры;
					           |- либо если это вызов метода регламентного задания, тогда указать обращение к свойству метаданных (можно через переменную, если необходимо: %1);
					           |- либо если это универсальный механизм, тогда вернуть имена вызываемых методов в процедуре %2, а само место вызова добавить в исключение (см. пример в подсистеме %3 или другой).'"),
					"Метаданные.РегламентныеЗадания[ИмяЗадания].ИмяМетода",
					"ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод",
					"СтандартныеПодсистемы.ОбновлениеВерсииИБ");
			КонецЕсли;
		Иначе
			Если ИмяМетодаРегламентногоЗадания = Неопределено Тогда
				ИмяМетодаВПараметре = Сред(Параметр, 2, СтрДлина(Параметр) - 2);
			Иначе
				ИмяМетодаВПараметре = ИмяМетодаРегламентногоЗадания;
			КонецЕсли;
			...
			Если ЧастиИмени.Количество() = 4 И ВРег(ЧастиИмени[2]) = "МОДУЛЬОБЪЕКТА" Тогда
				...
				ИмяПроцедурыВМодуле = ИмяМетодаВПараметре;
			...
			ИначеЕсли ЧастиИмени.Количество() < 2 Или ЧастиИмени.Количество() > 3 Тогда
				ТекстОшибки = НСтр("ru = 'Некорректное имя вызываемой процедуры'");
			Иначе
				ИмяПроцедурыВМодуле = ЧастиИмени[ЧастиИмени.ВГраница()];
			КонецЕсли;
			Если ТекстОшибки = Неопределено Тогда
				Если ДополнительныеПараметры.ИспользуемыеПроцедурыМодулей <> Неопределено Тогда
					ДобавитьИспользуемуюПроцедуру(ДополнительныеПараметры.ИспользуемыеПроцедурыМодулей,
						Неопределено, ИмяМетодаВПараметре, ТекущееУсловиеПоиска.ВФоне, Объект);
				КонецЕсли;
				Попытка
					...
					ТекстИсправленияОшибки = ТекстИсправленияОшибкиОтсутствияВставки(ИмяПроцедурыВМодуле,
						ТекущееУсловиеПоиска.ВФоне);
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ТекстОшибки + ?(ЗначениеЗаполнено(ТекстИсправленияОшибки), Символы.ПС + ТекстИсправленияОшибки, ""));
...
Функция ТекстИсправленияОшибкиОтсутствияВставки(ИмяПроцедурыВМодуле, ВФоне)
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для исправления нужно:
		           |а) найти следующую процедуру или вставить ее (например, в конце модуля):
		           |%1
		           |б) вставить в процедуру следующую строку:
		           |%2'"),
		"Процедура" + " " + "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы)" + " " + "Экспорт
		|КонецПроцедуры",
		СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(ИмяПроцедурыВМодуле, ВФоне));
	
КонецФункции

Функция СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(ИмяПроцедурыВМодуле, ВФоне)
	
	Возврат "	" + СтрШаблон("Методы.Вставить(""%1""%2);",
		ИмяПроцедурыВМодуле, ?(ВФоне = Истина, ", " + "Истина", ""));
...
Процедура ПроверитьИменаУказанныхМетодовМодуля(ТекстМодуля, ИмяФайла, ДополнительныеПараметры)
	...
	Для Каждого ОписаниеМетода Из Методы Цикл
		...
		МетодыДляПроверки.Вставить(ВРег(ИмяМетода), ОписаниеМетода);
	...
	Для Каждого КлючИЗначение Из МетодыДляПроверки Цикл
		ОписаниеМетода = КлючИЗначение.Значение;
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для исправления нужно в указанной процедуре модуля
			           |- удалить строку, подобную следующей:
			           |%1'"),
			СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(ОписаниеМетода.Ключ, ОписаниеМетода.Значение));
		ДобавитьОшибку(ДополнительныеПараметры.ОбъектМетаданных,
			НСтр("ru = 'Некорректное имя процедуры, указанной для вызова как произвольный код'"),
			ПодробноеОписаниеОшибки + Символы.ПС + ТекстИсправленияОшибки);
	КонецЦикла;
	
	ИспользуемыеПроцедурыМодулей = ДополнительныеПараметры.ИспользуемыеПроцедурыМодулей;
	Если ИспользуемыеПроцедурыМодулей = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОписаниеМетода Из Методы Цикл
		ИмяМетода = ОписаниеМетода.Ключ;
		Если МетодыДляПроверки.Получить(ВРег(ИмяМетода)) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ВРег(ИмяФайла) = ВРег("ObjectModule") Тогда
			ДобавитьИспользуемуюПроцедуру(ИспользуемыеПроцедурыМодулей,
				Неопределено, ИмяМетода, ОписаниеМетода.Значение = Истина, Модуль, Истина);
		Иначе
			ДобавитьИспользуемуюПроцедуру(ИспользуемыеПроцедурыМодулей,
				ПолноеИмя, ИмяМетода, ОписаниеМетода.Значение = Истина,, Истина);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьВызовыМетодовОбработчиковПодсистем(ИменаПроцедур, ЗаголовокОшибки, ИспользуемыеПроцедурыМодулей)
	...
	Для Каждого ОписаниеИмениПроцедуры Из ИменаПроцедур Цикл
		...
		Если ЧастиИмени.Количество() = 2 Тогда
			...
			ИмяПроцедурыВМодуле = ЧастиИмени[1];
		ИначеЕсли ЧастиИмени.Количество() = 3 Тогда
			...
			ИмяПроцедурыВМодуле = ЧастиИмени[2];
		Иначе
			Продолжить;
		КонецЕсли;
		Если ОбъектМетаданных = Неопределено Тогда
		...
		Если ИспользуемыеПроцедурыМодулей <> Неопределено Тогда
			ДобавитьИспользуемуюПроцедуру(ИспользуемыеПроцедурыМодулей,
				ОбъектМетаданных.ПолноеИмя(), ИмяПроцедурыВМодуле, ВФоне);
		КонецЕсли;
		ТекстОшибки = Неопределено;
		ТекстИсправленияОшибки = "";
		Попытка
			СтандартныеПодсистемыСервер.ПроверитьМетодДляВызоваКакПроизвольныйКод(ИмяПроцедуры, ВФоне,, ТекстОшибки);
			ТекстИсправленияОшибки = ТекстИсправленияОшибкиОтсутствияВставки(ИмяПроцедурыВМодуле, ВФоне);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
		...
		ДобавитьОшибку(ОбъектМетаданных,
			...
			+ ?(ЗначениеЗаполнено(ТекстИсправленияОшибки), Символы.ПС + ТекстИсправленияОшибки, ""));
	КонецЦикла;
	
КонецПроцедуры

Процедура ДобавитьИспользуемуюПроцедуру(ИспользуемыеПроцедурыМодулей, Знач ПолноеИмяМодуля, Знач ИмяПроцедуры,
			ВФоне, Объект = Неопределено, ЭтоНастройка = Ложь)
	
	Если Объект <> Неопределено Тогда
		ПолноеИмяМодуля = Объект.Метаданные().ПолноеИмя() + ".МодульОбъекта";
		
	ИначеЕсли ПолноеИмяМодуля = Неопределено Тогда
		ЧастиИмени = СтрРазделить(ИмяПроцедуры, ".");
		
		Если ЧастиИмени.Количество() = 2 Тогда
			ИмяПроцедуры = ЧастиИмени[1];
			МетаданныеМодуля = Метаданные.ОбщиеМодули.Найти(ЧастиИмени[0]);
			
		ИначеЕсли ЧастиИмени.Количество() = 3 Тогда
			ИмяПроцедуры = ЧастиИмени[2];
			Попытка
				КоллекцияМетаданных = Метаданные[ЧастиИмени[0]];
			Исключение
			...
			Если КоллекцияМетаданных = Неопределено Тогда
				Возврат;
			КонецЕсли;
			МетаданныеМодуля = КоллекцияМетаданных.Найти(ЧастиИмени[1]);
		Иначе
			Возврат;
		КонецЕсли;
		Если МетаданныеМодуля = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ПолноеИмяМодуля = МетаданныеМодуля.ПолноеИмя();
	КонецЕсли;
	
	ИменаПроцедур = ИспользуемыеПроцедурыМодулей.Получить(ПолноеИмяМодуля);
	
	Если ИменаПроцедур = Неопределено Тогда
		ИменаПроцедур = Новый ТаблицаЗначений;
		ИменаПроцедур.Колонки.Добавить("ИмяПроцедуры");
		ИменаПроцедур.Колонки.Добавить("ИмяПроцедурыВерхнийРегистр");
		ИменаПроцедур.Колонки.Добавить("ИспользованиеВФоне");
		ИменаПроцедур.Колонки.Добавить("НастройкаВФоне");
		ИменаПроцедур.Колонки.Добавить("ИсключениеВФоне");
		ИспользуемыеПроцедурыМодулей.Вставить(ПолноеИмяМодуля, ИменаПроцедур);
	КонецЕсли;
	
	ИмяПроцедурыВерхнийРегистр = ВРег(ИмяПроцедуры);
	СтрокаТЗ = ИменаПроцедур.Найти(ИмяПроцедурыВерхнийРегистр, "ИмяПроцедурыВерхнийРегистр");
	Если СтрокаТЗ = Неопределено Тогда
		СтрокаТЗ = ИменаПроцедур.Добавить();
		СтрокаТЗ.ИмяПроцедуры = ИмяПроцедуры;
		СтрокаТЗ.ИмяПроцедурыВерхнийРегистр = ИмяПроцедурыВерхнийРегистр;
	КонецЕсли;
	
	Если ЭтоНастройка = Ложь Тогда
		СтрокаТЗ.ИспользованиеВФоне = ВФоне Или СтрокаТЗ.ИспользованиеВФоне = Истина;
		
	ИначеЕсли ЭтоНастройка = Истина Тогда
		СтрокаТЗ.НастройкаВФоне = ВФоне;
	Иначе
		СтрокаТЗ.ИсключениеВФоне = ВФоне;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПроверитьИспользуемыеПроцедурыМодулей(ИспользуемыеПроцедурыМодулей)
	
	Для Каждого КлючИЗначение Из ИспользуемыеПроцедурыМодулей Цикл
		ИменаПроцедур = КлючИЗначение.Значение;
		ПолноеИмя = КлючИЗначение.Ключ;
		Если СтрЗаканчиваетсяНа(ПолноеИмя, ".МодульОбъекта") Тогда
			ПолноеИмя = Сред(ПолноеИмя, 1, СтрДлина(ПолноеИмя) - СтрДлина(".МодульОбъекта"));
		КонецЕсли;
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмя);
		Для Каждого СтрокаТЗ Из ИменаПроцедур Цикл
			Если СтрокаТЗ.ИспользованиеВФоне = Неопределено
			   И СтрокаТЗ.НастройкаВФоне <> Неопределено
			   И СтрокаТЗ.ИсключениеВФоне = Неопределено Тогда
				
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: не найден вызов процедуры или функции %2, указанной в процедуре %3.'"),
					КлючИЗначение.Ключ,
					СтрокаТЗ.ИмяПроцедуры,
					"ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод");
				ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для исправления нужно в указанной процедуре модуля
					           |- удалить строку, подобную следующей:
					           |%1'"),
					СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(СтрокаТЗ.ИмяПроцедуры, СтрокаТЗ.НастройкаВФоне));
				ДобавитьОшибку(ОбъектМетаданных,
					НСтр("ru = 'Неиспользуемая процедура для вызова, как произвольный код'"),
					ПодробноеОписаниеОшибки + Символы.ПС + ТекстИсправленияОшибки);
				
			ИначеЕсли СтрокаТЗ.ИспользованиеВФоне = Ложь
			        И СтрокаТЗ.НастройкаВФоне = Истина
			        И СтрокаТЗ.ИсключениеВФоне <> Истина Тогда
				
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: не найден вызов в фоне процедуры или функции %2, указанной в процедуре %3.'"),
					КлючИЗначение.Ключ,
					СтрокаТЗ.ИмяПроцедуры,
					"ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод");
				ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для исправления нужно в указанной процедуре модуля
					           |- заменить строку, подобную следующей:
					           |%1
					           |- на строку:
					           |%2'"),
					СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(СтрокаТЗ.ИмяПроцедуры, Истина),
					СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(СтрокаТЗ.ИмяПроцедуры, Ложь));
				ДобавитьОшибку(ОбъектМетаданных,
					НСтр("ru = 'Неиспользуемая процедура для вызова, как произвольный код'"),
					ПодробноеОписаниеОшибки + Символы.ПС + ТекстИсправленияОшибки);
				
			ИначеЕсли СтрокаТЗ.НастройкаВФоне = Неопределено
			        И СтрокаТЗ.ИсключениеВФоне <> Неопределено Тогда
				
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: вызов процедуры или функции %2 не указан в процедуре %3, но указан в одной из процедур %4.'"),
					КлючИЗначение.Ключ,
					СтрокаТЗ.ИмяПроцедуры,
					"ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод",
					"ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод");
				ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для исправления нужно:
					           |- найти строку, подобную следующей, в одной из процедур %1 (это настройки только для отчета):
					           |%2
					           |- и удалить строку настройки методов, подобную следующей:
					           |%3'"),
					"ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод",
					СтрШаблон("	" + "Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить(""%1"","
						+ " " + "Методы);", КлючИЗначение.Ключ),
					СтрокаВставкиМетодаВызываемогоКакПроизвольныйКод(СтрокаТЗ.ИмяПроцедуры, СтрокаТЗ.ИсключениеВФоне));
				ДобавитьОшибку(ОбъектМетаданных,
					НСтр("ru = 'Лишнее исключение проверки использования процедуры для вызова, как произвольный код'"),
					ПодробноеОписаниеОшибки + Символы.ПС + ТекстИсправленияОшибки);
				
			ИначеЕсли СтрокаТЗ.НастройкаВФоне = Ложь
			        И СтрокаТЗ.ИсключениеВФоне = Истина Тогда
				
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1: вызов в фоне процедуры или функции %2 не указан в процедуре %3, но указан в одной из процедур %4.'"),
					КлючИЗначение.Ключ,
					СтрокаТЗ.ИмяПроцедуры,
					"ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод",
					"ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод");
				ТекстИсправленияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для исправления нужно:
					           |- найти строку, подобную следующей, в одной из процедур %1 (это настройки только для отчета):
					           |%2
					           |- и заменить строку настройки методов, подобную следующей:
					           |%3
					           |- на строку, подобную следующей:
					           |%4'"),
					"ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод",
					СтрШаблон("	" + "Настройки.ИсключенияКогдаУказанныйМетодНеВызывается.Вставить(""%1"","
						+ " " + "Методы);", КлючИЗначение.Ключ),
...
Функция ОбъектыИсключенияПланаОбменаЛюбогоРИБ()
	...
	ДобавитьОбъект("РегистрСведений.ДействияПриПолученииДанныхОбмена", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.КоличествоБлокируемыхОбъектовКЗагрузке", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.ОбъектыБлокирующиеЗагрузку", Исключения);
	ДобавитьОбъект("РегистрСведений.ОбъектыКЗагрузке", Исключения);
	ДобавитьОбъект("РегистрСведений.ПравилаДляОбменаДанными", Исключения);
	ДобавитьОбъект("РегистрСведений.ПрочитанныеДанныеСообщенийОбмена", Исключения);
	...
	ДобавитьОбъект("Справочник.ОбщиеНастройкиАвторизацииИнтернетСервисов", Исключения);
	...
	ДобавитьОбъект("Константа.ИспользоватьФайловыйАрхив", Исключения);
	ДобавитьОбъект("Константа.КаталогРезервныхКопийВстроенногоХранилищаДвоичныхДанныхДляLinux", Исключения);
	ДобавитьОбъект("Константа.КаталогРезервныхКопийВстроенногоХранилищаДвоичныхДанныхДляWindows", Исключения);
	ДобавитьОбъект("Константа.РазмерПорцииПеремещаемыхФайлов", Исключения);
	ДобавитьОбъект("Константа.ТекстИнформированияПользователяОНедоступностиФайлаВАрхиве", Исключения);
	ДобавитьОбъект("РегистрСведений.ИсторияРезервногоКопированияВстроенногоХранилищаДвоичныхДанных", Исключения);
	ДобавитьОбъект("РегистрСведений.МестаХраненияДвоичныхДанных", Исключения);
	ДобавитьОбъект("РегистрСведений.НастройкиРаботыСФайловымАрхивом", Исключения);
	ДобавитьОбъект("РегистрСведений.СведенияОХраненииДедуплицированныхФайлов", Исключения);
	...
	ДобавитьОбъект("Константа.ИспользоватьГосключ", Исключения);
	ДобавитьОбъект("Константа.ИспользоватьСервисМобильнойПодписи", Исключения);
	ДобавитьОбъект("РегистрСведений.СервисМобильнойПодписиСтатусы", Исключения);
	ДобавитьОбъект("Справочник.ПодписантыСервисаМобильнойПодписи", Исключения);
	ДобавитьОбъект("Справочник.НастройкиОтправкиВГосключ", Исключения);
...
Процедура ПроверитьПрямоеОбращениеКПараметрамСеанса()
	...
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		...
		ТекстМодуля = ПрочитатьТекстМодуля(ФайлМодуля.ПолноеИмя);
...
Процедура ПроверитьИспользованиеОграниченийДоступа(ОграниченияДоступа)
	...
	ОпределитьВидыОграниченийПрав(ВидыОграниченийПрав, ОграниченияДоступа);
...
Процедура ОпределитьВидыОграниченийПрав(ВидыОграниченийПрав, ОграниченияДоступа)
...
Процедура СравнитьОписанияВидовОграниченийПрав(СуществующееОписание, НовоеОписание)
	...
	Для НомерСтроки = 1 По ЧислоСтрок Цикл
		...
		Если СтрНайти(СуществующееОписание, Сред(Строка, 3)) = 0 И ЗначениеЗаполнено(Строка) Тогда
	...
	Для НомерСтроки = 1 По ЧислоСтрок Цикл
		...
		Если СтрНайти(НовоеОписание, СокрП("	|" + Строка)) = 0 И ЗначениеЗаполнено(Строка) Тогда
...
// Только для внутреннего использования.
Процедура ПроверитьНаличиеОграниченияПоВсемПолям(ОграниченияДоступа)
	
	// Проверка использования ограничения по всем полям.
...
//     ** ОтключенСтандартныйВариант - Булево
//
Функция ПараметрыПроверкиВнедренияУправленияДоступом()
	...
	Параметры.УправлениеДоступом.Вставить("ШаблоныОграничений", Неопределено);
	...
	Параметры.УправлениеДоступом.Вставить("ОтключенСтандартныйВариант", Неопределено);
...
//  Соответствие
//
Функция ЭталонныеШаблоныОграничений(Параметры)
	...
	Если Параметры.УправлениеДоступом.ОтключенСтандартныйВариант Тогда
		УстаревшиеШаблоны = Новый Массив;
		УстаревшиеШаблоны.Добавить("ПоЗначениям");
		УстаревшиеШаблоны.Добавить("ПоЗначениямРасширенный");
		УстаревшиеШаблоны.Добавить("ПоЗначениямИНаборамРасширенный");
		УстаревшиеШаблоны.Добавить("ПоНаборамЗначений");
		Для Каждого ИмяШаблона Из УстаревшиеШаблоны Цикл
			Результат.Вставить(ИмяШаблона, Новый Структура("Имя,Текст", ИмяШаблона, ТекстУстаревшегоШаблона()));
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстУстаревшегоШаблона()
	
	Возврат "УстаревшийШаблон"; // АПК:1297 Для разработчика (должно быть без НСтр).
	...
	Пока Объект <> Неопределено Цикл
		...
		Если ОбъектМетаданных <> Неопределено Тогда
			...
			Иначе
				...
				КонецЕсли;
			КонецЕсли;
			Если ОписаниеОграничения <> Неопределено
			 Или Параметры.УправлениеДоступом.ОтключенСтандартныйВариант Тогда
				
				НовыйТекстОграничения = ?(ОписаниеОграничения <> Неопределено,
					ТекстОграниченияИзОписания(ОписаниеОграничения), Неопределено);
				...
				Пока Право <> Неопределено Цикл
					...
					Если ОграничениеДоступа <> Неопределено И Не ПустаяСтрока(ОграничениеДоступа.ТекстовоеСодержимое)
						И (    СтрНайти(ОграничениеДоступа.ТекстовоеСодержимое, "ОбязательноеДополнительноеУсловиеНачало") = 0
						     И СтрНайти(ОграничениеДоступа.ТекстовоеСодержимое, "ОбязательноеДополнительноеУсловиеКонец") = 0
						   Или Параметры.УправлениеДоступом.ОтключенСтандартныйВариант) Тогда
						
						Если Не Параметры.УправлениеДоступом.ОтключенСтандартныйВариант Тогда
							ТекстОграниченияДляВставки = ТекстОграниченияДляВставки(НовыйТекстОграничения, ОграничениеДоступа.ТекстовоеСодержимое);
						ИначеЕсли НовыйТекстОграничения <> Неопределено
						        И СтрНайти(ОграничениеДоступа.ТекстовоеСодержимое, "ОбязательноеДополнительноеУсловиеНачало") = 0
						        И СтрНайти(ОграничениеДоступа.ТекстовоеСодержимое, "ОбязательноеДополнительноеУсловиеКонец") = 0 Тогда
							ТекстОграниченияДляВставки = НовыйТекстОграничения;
						Иначе
							ТекстОграниченияДляВставки = ТекстОграниченияБезСтарогоRLS(ОграничениеДоступа.ТекстовоеСодержимое);
						КонецЕсли;
...
Процедура ПроверитьСоответствиеСоставаСписковСОграничениемНаличиюRLSВРолях(ОграниченияДоступа,
			СпискиСОграничениемДоступа)
	...
	ТаблицыСОграничением = Новый ТаблицаЗначений;
	ТаблицыСОграничением.Колонки.Добавить("ОбъектМетаданных");
	ТаблицыСОграничением.Колонки.Добавить("РолиСОграничением");
	...
	Для Каждого Таблица Из СписокТаблиц Цикл
		...
		РолиСОграничением = Новый Массив;
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если Не ТекстОграниченияСодержитСтандартныйШаблонRLS(НайденнаяСтрока.Ограничение) Тогда
				Продолжить;
			КонецЕсли;
			Если РолиСОграничением.Найти(НайденнаяСтрока.Роль) = Неопределено Тогда
				РолиСОграничением.Добавить(НайденнаяСтрока.Роль);
			КонецЕсли;
		КонецЦикла;
		Если ЗначениеЗаполнено(РолиСОграничением) Тогда
			СтрокаТЗ = ТаблицыСОграничением.Добавить();
			СтрокаТЗ.ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Таблица);
			СтрокаТЗ.РолиСОграничением = РолиСОграничением;
		КонецЕсли;
	КонецЦикла;
	
	ШаблонДляВставки = "	Списки.Вставить(%1, Истина);";
	
	Для Каждого СтрокаТЗ Из ТаблицыСОграничением Цикл
		ОбъектМетаданных = СтрокаТЗ.ОбъектМетаданных;
		Если СпискиСОграничениемДоступа[ОбъектМетаданных] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		РолиСОграничением = СтрокаТЗ.РолиСОграничением;
		
		ТекстПояснения = "";
		Если РолиСОграничением.Количество() = 1 Тогда
			ТекстПояснения = НСтр("ru = 'Доступ к объекту на уровне записей ограничен в роли'") + " " + РолиСОграничением[0];
		Иначе
			ТекстПояснения = НСтр("ru = 'Доступ к объекту на уровне записей ограничен в ролях:'") + Символы.ПС 
				+ СтрСоединить(РолиСОграничением, Символы.ПС);
		КонецЕсли;
		...
		ПодробноеОписаниеОшибки = НСтр("ru = 'В процедуре %1 отсутствует обязательная вставка кода:'") + Символы.ПС
			+ ТекстДляВставки + Символы.ПС + Символы.ПС
			+ ТекстПояснения;
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодробноеОписаниеОшибки,
...
Функция ТекстОграниченияСодержитСтандартныйШаблонRLS(ТекстОграничения)
	
	Возврат СтрНайти(ТекстОграничения, "#ДляОбъекта") > 0
		Или СтрНайти(ТекстОграничения, "#ДляРегистра") > 0
		Или СтрНайти(ТекстОграничения, "#ПоЗначениям") > 0
		Или СтрНайти(ТекстОграничения, "#ПоЗначениямРасширенный") > 0
		Или СтрНайти(ТекстОграничения, "#ПоЗначениямИНаборамРасширенный") > 0
		Или СтрНайти(ТекстОграничения, "#ПоНаборамЗначений") > 0;
...
Процедура ПроверитьНаличиеШаблонаВРоли(ДокументDOM, Роль, ИмяШаблона, ПоставляемыйШаблон, ЕстьИзменения)
	...
	ТекстУстаревшегоШаблона = ТекстУстаревшегоШаблона();
	
	Если Не ШаблонИспользуется
	   И Роль.Имя <> "ИзменениеУчастниковГруппДоступа"
	 Или ПоставляемыйШаблон.Текст = ТекстУстаревшегоШаблона Тогда
		
		Если НайденныйШаблон = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ПоставляемыйШаблон.Текст = ТекстУстаревшегоШаблона,
				НСтр("ru = 'Шаблон #%1 не используется в конфигурации.'"),
		...
		Если ИсправлятьОшибку("ШаблонНеИспользуетсяВРоли") Тогда
			...
			ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				?(ПоставляемыйШаблон.Текст = ТекстУстаревшегоШаблона,
					НСтр("ru = 'Исправлено. Неиспользуемый в конфигурации шаблон #%1 удален.'"),
					НСтр("ru = 'Исправлено. Неиспользуемый в роли шаблон #%1 удален.'")), ИмяШаблона));
		Иначе
			ДобавитьОшибку(Роль, ЗаголовокОшибки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				?(ПоставляемыйШаблон.Текст = ТекстУстаревшегоШаблона,
					НСтр("ru = 'Шаблон #%1 не используется в конфигурации, удалите его.'"),
					НСтр("ru = 'Шаблон #%1 не используется в роли, удалите его.'")), ИмяШаблона));
		КонецЕсли;
		Возврат;
	КонецЕсли;
	
	Если Не ТребуетсяВыполнениеПроверки("ИспользуемыйШаблонВРолиОтсутствуетИлиОтличается")
	 Или Роль.Имя = "ИзменениеУчастниковГруппДоступа"
	 Или ПоставляемыйШаблон.Текст = ТекстУстаревшегоШаблона Тогда
		Возврат;
	КонецЕсли;
...
Функция ТекстОграниченияБезСтарогоRLS(Знач ТекстОграничения)
	
	ТекстОграничения = СокрЛП(ТекстОграничения);
	ТекстНачала = "#Если &ОграничениеДоступаНаУровнеЗаписейУниверсально #Тогда";
	
	Если СтрНачинаетсяС(ВРег(ТекстОграничения), ВРег(ТекстНачала)) Тогда
		ТекстДляПоиска = "#Иначе";
		Позиция = СтрНайти(ВРег(ТекстОграничения), ВРег(ТекстДляПоиска));
		Если Позиция > 0 Тогда
			ТекстОграничения = Сред(ТекстОграничения, СтрДлина(ТекстНачала) + 1, Позиция - СтрДлина(ТекстНачала) - 1);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьПроверку(ПараметрыПроверки = Неопределено)
	...
	ВыполнитьПроверкуВНесколькоПотоков(ПроверяемыеПодсистемы,
...
Функция ТаблицаДопустимыхВызововБСПВОбработчикахСобытийКорневыхМодулейКонфигурации()
	...
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульОбычногоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПередЗавершениемРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "СтандартныеПодсистемыКлиент.ПередЗавершениемРаботыСистемы(Отказ, ТекстПредупреждения);";
...
//                                                         объекта отчета.
//   Проверка                        - Строка            - имя проверки, пустая строка, если основная проверка.
//
// Возвращаемое значение:
...
//
Функция РезультатПроверкиПодсистемы(ПодсистемаБСП, ХранилищеСтруктурыИнициализации, Проверка = "") Экспорт
	...
	Если ПроверяемаяПодсистема <> Неопределено Тогда
		Если ЗначениеЗаполнено(Проверка) Тогда
			...
			ИмяПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1(%2)", ПроверкиПодсистемыУправлениеДоступом[Проверка], "ПроверяемыеОбъекты");
		Иначе
			ИмяПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1_%2_%3()", "Подключаемый", ПодсистемаБСП, "ПроверитьВнедрение");
		КонецЕсли;
		
		Попытка
			Выполнить(ИмяПроцедуры); // АПК:486 - Не нарушает безопасность информации, так как выполняются только проверенные процедуры модуля.
		Исключение
	...
	Иначе
		ДобавитьОшибку(ПроверяемаяПодсистема, НСтр("ru = 'Проверка внедрения подсистемы не выполнена:'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдена подсистема БСП ""%1"".'"), ПодсистемаБСП));
	КонецЕсли;
...
Процедура ВыполнитьПроверкуВНесколькоПотоков(ПроверяемыеПодсистемы, БезРасширений)
	...
	Если ЭтоВнешнийОтчет Тогда
		...
		КонецЕсли;
		ОписаниеВнешнегоОтчета = ДлительныеОперации.НовоеОписаниеВнешнегоОтчетаОбработки();
		ОписаниеВнешнегоОтчета.Данные         = ДвоичныеДанныеВнешнегоОтчета;
		ОписаниеВнешнегоОтчета.ПолноеИмяФайла = ПолноеИмяФайлаВнешнегоОтчета;
		ПараметрыВыполнения.ВнешнийОтчетОбработка = ОписаниеВнешнегоОтчета;
	КонецЕсли;
	...
	Для Каждого Подсистема Из ПроверяемыеПодсистемы Цикл
		...
		Если ПроверяемаяПодсистема <> Неопределено Тогда
			ДобавитьПараметрМетодаМногопоточнойОперации(ПараметрыМетода, ИмяПодсистемы,
...
Процедура ДобавитьПараметрМетодаМногопоточнойОперации(ПараметрыМетода, ИмяПодсистемы, ХранилищеСтруктурыИнициализации)
	
	МассивПараметров = Новый Массив;
	МассивПараметров.Добавить(ИмяПодсистемы);
	МассивПараметров.Добавить(ХранилищеСтруктурыИнициализации);
	
	ПараметрыМетода.Вставить(ИмяПодсистемы, МассивПараметров);
	...
	Для Каждого СведенияОПроверке Из ПроверкиПодсистемыУправлениеДоступом Цикл
		...
		ПараметрыМетода.Вставить(ИмяПодсистемы + "_" + СведенияОПроверке.Ключ, МассивПараметров);
	КонецЦикла;
