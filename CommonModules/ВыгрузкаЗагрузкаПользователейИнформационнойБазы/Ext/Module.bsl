...
Процедура ЗагрузитьПользователейИнформационнойБазы(Контейнер) Экспорт
	...
	Пока ПотокЧтения.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		СериализацияПользователя = ФабрикаXDTO.ПрочитатьXML(
			ПотокЧтения, ФабрикаXDTO.Тип("http://www.1c.ru/1cFresh/Data/Dump/1.0.2.1", "InfoBaseUser"));
		
		ПользовательИБ = ДесериализоватьПользователяИнформационнойБазы(СериализацияПользователя);
		
		Если НетПользователейСАдминистративнымиПравами
			И Не (ПользовательИБ.АутентификацияСтандартная И ЗначениеЗаполнено(ПользовательИБ.Роли)) Тогда
...
Функция СортироватьМассивПользователейИБПередВыгрузкой(Знач ИсходныйМассив)
	...
	ТаблицаПользователей.Колонки.Добавить("Пользователь", Новый ОписаниеТипов("ПользовательИнформационнойБазы"));
	ТаблицаПользователей.Колонки.Добавить("Администратор", Новый ОписаниеТипов("Булево"));
	ТаблицаПользователей.Колонки.Добавить("АутентификацияСтандартная", Новый ОписаниеТипов("Булево"));
	
	Для Каждого ПользовательИБ Из ИсходныйМассив Цикл
		
		СтрокаТЗ = ТаблицаПользователей.Добавить();
	...
	ТаблицаПользователей.Сортировать("Администратор Убыв, АутентификацияСтандартная Убыв");
