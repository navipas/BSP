...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	// Конец СтандартныеПодсистемы.ЗащитаПерсональныхДанных
	ЗаполнитьПрофильПодписаниеИШифрованиеЛичнымиСертификатами(ОписанияПрофилей);
	ЗаполнитьПрофильПодписаниеИШифрованиеЛичнымиСертификатамиСДобавлениемСертификатов(ОписанияПрофилей);
...
Процедура ПриЗаполненииРеквизитовОрганизацииПодписантаСервисаМобильнойПодписи(РеквизитыОрганизации, СтандартнаяОбработка) Экспорт
	...
	Объекты = Новый Массив;
	Объекты.Добавить(РеквизитыОрганизации.Организация);
	...
	Строка = КонтактнаяИнформация.Найти(УправлениеКонтактнойИнформацией.ВидКонтактнойИнформацииПоИмени("_ДемоЮридическийАдресОрганизации"), "Вид");
	Если Строка <> Неопределено Тогда
		СведенияОбАдресе = УправлениеКонтактнойИнформацией.СведенияОбАдресе(Строка.Значение, Новый Структура("КодыАдреса", Истина));
		Если СведенияОбАдресе.Свойство("ДополнительныеКоды") И СведенияОбАдресе.ДополнительныеКоды.Свойство("ОКАТО") Тогда
			РеквизитыОрганизации.КодОКАТО = СведенияОбАдресе.ДополнительныеКоды.ОКАТО;
		Иначе
			РеквизитыОрганизации.КодОКАТО = СведенияОбАдресе.КодРегиона;
			Пока СтрДлина(РеквизитыОрганизации.КодОКАТО) < 11  Цикл
				РеквизитыОрганизации.КодОКАТО = РеквизитыОрганизации.КодОКАТО + "0";
			КонецЦикла;
		КонецЕсли;
...
Процедура ПриЗаполненииРеквизитовФизическогоЛицаПодписантаСервисаМобильнойПодписи(РеквизитыФизическогоЛица) Экспорт
	...
	ДанныеСсылки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыФизическогоЛица.ФизическоеЛицо,
		"ЭтоГруппа, ИНН, Фамилия, Имя, Отчество, СНИЛС");
	Если ДанныеСсылки.ЭтоГруппа Тогда
		Возврат;
	КонецЕсли;
	...
	РеквизитыФизическогоЛица.СНИЛС = ДанныеСсылки.СНИЛС;
...
Процедура ЗаполнитьПрофильРуководитель(Знач ОписанияПрофилей)
	...
	// СтандартныеПодсистемы.СервисМобильнойПодписи
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеПодписантовМобильногоСервиса");
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеНастроекОтправкиВГосключ");
	// Конец СтандартныеПодсистемы.СервисМобильнойПодписи
	// СтандартныеПодсистемы.ЭлектроннаяПодписьСервисаDSS
	ОписаниеПрофиля.Роли.Добавить("ЧтениеДанныхСервисаDSS");
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеУчетныхЗаписейСерверовDSS");
...
Процедура ЗаполнитьПрофильПодписаниеИШифрованиеЛичнымиСертификатами(Знач ОписанияПрофилей)
	
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Имя           = "_ДемоПодписаниеИШифрованиеЛичнымиСертификатами";
	ОписаниеПрофиля.Родитель      = "ДополнительныеПрофили";
	ОписаниеПрофиля.Идентификатор = "4a89f588-f04f-11f0-bf80-e89c25df8ea7";
	ОписаниеПрофиля.Наименование  =
		НСтр("ru = 'Подписание и шифрование личными сертификатами (дополнительно)'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Описание =
		НСтр("ru = 'Дополнительно назначается пользователям, которым должны быть доступны подписание и шифрование данных.
			|Сертификаты доступны только для чтения и могут быть ограничены согласно настройке прав.'");
...
Процедура ЗаполнитьПрофильПодписаниеИШифрованиеЛичнымиСертификатамиСДобавлениемСертификатов(Знач ОписанияПрофилей)
	
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Имя           = "_ДемоПодписаниеИШифрованиеЛичнымиСертификатамиСДобавлениемСертификатов";
	ОписаниеПрофиля.Родитель      = "ДополнительныеПрофили";
	ОписаниеПрофиля.Идентификатор = "91c37829-16e2-4d0a-8c77-10489d83ed82";
	ОписаниеПрофиля.Наименование  =
		НСтр("ru = 'Подписание и шифрование личными сертификатами с добавлением сертификатов (дополнительно)'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Описание =
		НСтр("ru = 'Дополнительно назначается пользователям, которым должны быть доступны подписание и шифрование данных.
			|Сертификаты могут быть ограничены согласно настройке прав.'");
	
	// Основные возможности профиля.
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеЛичныхСертификатовКлючейЭлектроннойПодписиИШифрования");
	ОписаниеПрофиля.Роли.Добавить("ДобавлениеИзменениеЭлектронныхПодписей");
	ОписаниеПрофиля.Роли.Добавить("ШифрованиеИРасшифровкаДанных");
